...
/**
 * Black-box tests for TraitGroup: property-based group membership,
 ...
 */
internal class TraitGroupTest : AbcTraitGroupTest() {
    ...
    fun `addGroupNode first call initializes global counter from zero`() {
        registerGroup("items")
        val meta = graph.storage.getMeta(TraitGroup.META_GLOBAL_COUNTER)
        assertNull(meta)
    ...
    fun `rebuildGroupCaches skips node with group but no suffix`() {
        ...
        node[TraitGroup.PROP_GROUP] = "data".strVal
    ...
    fun `rebuildGroupCaches persisted counter zero`() {
        ...
        node[TraitGroup.PROP_GROUP] = "grp".strVal
        node[TraitGroup.PROP_SUFFIX] = "s1".strVal
