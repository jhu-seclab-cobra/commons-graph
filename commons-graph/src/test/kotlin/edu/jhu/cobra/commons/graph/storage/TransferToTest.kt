...
class TransferToTest {
    ...
    fun `test transferTo copies edges and properties`() {
        ...
        source.addEdge(node1, node2, StorageTestUtils.EDGE_TAG_1, mapOf("weight" to 1.5.numVal))
        source.addEdge(node2, node3, StorageTestUtils.EDGE_TAG_2, mapOf("label" to "x".strVal))
    ...
    fun `test transferTo does not modify source`() {
        ...
        val node2 = source.addNode()
        val edge1 = source.addEdge(node1, node2, StorageTestUtils.EDGE_TAG_1)
