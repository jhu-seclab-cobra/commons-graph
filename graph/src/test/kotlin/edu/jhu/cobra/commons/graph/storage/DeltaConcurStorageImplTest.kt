...
class DeltaConcurStorageImplTest {
    ...
    private lateinit var storage: DeltaConcurStorageImpl
    ...
    fun setup() {
        ...
        storage = DeltaConcurStorageImpl(base)
    }
    ...
    @Test
    fun `test getOutgoingEdges returns union when no deleted edges`() {
        base.addNode(node1)
        base.addNode(node2)
        base.addEdge(edge1)
    ...
    @Test
    fun `test concurrent reads do not interfere`() {
        base.addNode(node1, mapOf("a" to "1".strVal))
        base.addNode(node2, mapOf("b" to "2".strVal))
            ...
            (1..10).map {
                Thread {
                    repeat(100) {
                        ...
                        storage.getNodeProperties(node2)
                    }
                }
            }
    ...
    @Test
    fun `test concurrent writes are serialized`() {
        val threads =
            (1..10).map { i ->
                Thread {
                    val nid =
                        edu.jhu.cobra.commons.graph.NodeID("concurrent_node_$i")
                    storage.addNode(nid, mapOf("idx" to i.numVal))
                }
            }
        threads.forEach { it.start() }
        threads.forEach { it.join() }
