...
## Node ID Generation (TraitGroup, deprecated)

### Problem

Generate unique opaque node IDs for grouped nodes. Correct when no two IDs collide.

### Steps

1. Read global counter from storage meta. Default to 0 if absent.
2. Increment the global counter.
3. Generate node ID as `"{prefix}_{counter}"`.
4. Persist the new counter value to storage meta.
5. Create node with the generated ID.

### Invariants

- Global counter is monotonically increasing. Per-group counters likewise.
- Counter persistence survives serialization/deserialization.

### Termination

Fixed sequence of five steps. No loops or recursion.

### Complexity

- Time: O(1). Space: O(G) where G = number of groups.
