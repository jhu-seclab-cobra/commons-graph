...
Benchmarked on macOS (Apple Silicon), GraalVM CE 21.0.2 (Graal JIT), G1GC with `-XX:MaxGCPauseMillis=200 -XX:G1HeapRegionSize=32m`.
Each metric is the **median of 3 independent JVM invocations**.
...
./gradlew :graph:test --tests "*.OptimizationBenchmarkTest" -PincludePerformanceTests --rerun
```
...
> generated by `addNode`/`addEdge`. AbcMultipleGraph maintains `nodeIdCache: HashMap<NodeID, InternalID>` for uname→storageId resolution.

### Graph-Level Query Operations (10K nodes, 50K edges, 100K queries)

| Storage | getNode | getOutEdges | getChildren | getDescendants |
|---|---|---|---|---|
| NativeStorage | 63.82M | 4.78M | 3.79M | 293.2K |
| NativeConcurStorage | 87.44M | 5.50M | 3.84M | 633.1K |
| LayeredStorage | 71.74M | 4.73M | 3.19M | 561.9K |

### Graph Population (median ms)

| Storage | 10K/30K | 100K/300K |
|---|---|---|
| NativeStorage (MultipleGraph) | 10.4 | 150.3 |
| NativeConcurStorage (MultipleGraph) | 7.4 | 131.8 |
| LayeredStorage (MultipleGraph) | 7.0 | 139.2 |
| NativeStorage (SimpleGraph) | — | 194.2 |
| NativeConcurStorage (SimpleGraph) | — | 163.6 |
| LayeredStorage (SimpleGraph) | — | 209.6 |

### Storage-Level Operations

#### Node Operations

| Implementation | Lookup (500K ops) | Delete (2K from 10K) |
|---|---|---|
| NativeStorageImpl | 62.52M | 1.51M |
| NativeConcurStorageImpl | 23.92M | 1.82M |
| LayeredStorageImpl | 87.90M | 2.03M |

#### Edge Operations

| Implementation | Outgoing Query | Incoming Query |
|---|---|---|
| NativeStorageImpl | 74.44M | 52.61M |
| NativeConcurStorageImpl | 27.01M | 19.30M |
| LayeredStorageImpl | 42.83M | 15.16M |
...
|---|---|---|
| NativeStorageImpl | 46.85M | 20.45M |
| NativeConcurStorageImpl | 21.81M | 7.06M |
| LayeredStorageImpl | 16.24M | 14.54M |
...
|---|---|---|---|
| NativeStorageImpl | 6.0 | 144.3 | 1876.9 |
| NativeConcurStorageImpl | 6.3 | 106.7 | 1839.5 |
| LayeredStorageImpl | 7.0 | 141.4 | 2361.0 |
...
|---|---|---|
| NativeStorage | 6.1 | 983.7K |
| NativeConcurStorage | 3.8 | 1.08M |
| LayeredStorage | 2.9 | 923.4K |

### Fixpoint State Access (P0, 50K nodes, 200K iterations)

| Approach | Read | Write |
|---|---|---|
| IStorage (HashMap) | 51.43M | 23.28M |
| Direct array (simulated) | 99.82M | 109.00M |
| Speedup | 1.9x | 4.7x |

> ⚠ P0 numbers are heavily affected by JIT noise. Direct array read shows extreme variance
> (99.82M to 425.46M across runs). Speedup ratios are unreliable.

### Memory (50K nodes, 150K edges)

| Metric | MB |
|---|---|
| Storage (with metadata) | 116.6 |
| Storage + graph layer | 120.5 |
| Graph layer overhead | 3.9 |

### Fixpoint Memory (P0, 100K nodes)

| Approach | MB |
|---|---|
| IStorage (HashMap per node) | 44.2 |
| Direct Array\<IValue\> | 3.4 |
| Savings | 12.9x |

### Optimization Baselines (OptimizationBenchmarkTest)

#### B4/B5: Cross-Layer Query Overhead (10K nodes/layer, 5 props/entity, 200K queries)

| Layers | containsNode | getNodeProps | getNodeProp | getOutEdges |
|---|---|---|---|---|
| 1 | 81.22M | 58.30M | 25.93M | 45.78M |
| 2 | 67.83M | 25.00M | 16.45M | 8.69M |
| 5 | 10.04M | 14.66M | 8.98M | 15.06M |
| 10 | 21.21M | 13.55M | 8.43M | 11.32M |

#### B6: Edge Query — NativeStorage vs LayeredStorage (10K nodes, 5 edges/node, 200K queries)

| Implementation | getOutEdges | getInEdges | slowdown |
|---|---|---|---|
| NativeStorageImpl | 72.38M | 104.40M | — |
| LayeredStorage (2 layers) | 14.68M | 12.92M | 4.9x / 8.1x |

#### Baseline NativeStorage Read Speed (50K nodes, 3 edges/node, 5 props/entity, 200K ops)

| Implementation | getNodeProps | getNodeProp | containsNode | getOutEdges |
|---|---|---|---|---|
| NativeStorageImpl | 41.17M | 15.19M | 67.31M | 40.01M |
| NativeConcurStorageImpl | 17.33M | 9.30M | 12.92M | 9.87M |

#### B2: Memory Footprint (3 edges/node, 5 props/entity)
...
### B1-B: Integer Edge Indexing (Implemented)

`IStorage` now uses `Int`-based IDs natively (`typealias InternalID = Int`). Node/edge IDs are auto-incrementing integers. Graph layer overhead reduced from 6.1 MB to 3.9 MB (50K nodes, 150K edges). Further savings possible with eclipse-collections `IntObjectHashMap` to eliminate Integer autoboxing in generic HashMap.
