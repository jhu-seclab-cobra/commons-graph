...
- Label poset hierarchy is defined by `IPartialOrderSet` (see `docs/core/label.design.md`); `IGraph` only consumes labels via label-filtered query overloads.
...
Label lattice functionality is defined by `IPartialOrderSet` and implemented in `AbcMultipleGraph` with write-through persistence to `IStorage` metadata. See `docs/core/label.design.md` for details.
