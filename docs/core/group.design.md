...
- **Abstract**: `TraitNodeGroup` (implemented by concrete graph classes alongside `AbcSimpleGraph` / `AbcMultiGraph`)
...
`TraitNodeGroup` is an optional graph trait that adds structured group-scoped node naming on top of standard `IGraph` operations. It owns a group registry with monotonically increasing per-group counters, structured `NodeID` generation with the format `graphName@group#suffix`, and convenience methods for creating and retrieving group-scoped nodes. It does **not** own storage, property management, or graph traversal — all operations delegate to the underlying `IGraph` implementation.
...
```
<graphName>@<group>#<suffix>
```

- `graphName` — the graph's name with `@` chars stripped (prevents ambiguous splitting)
...
|--------|----------|-------|--------|--------|
| `addGroupNode(group, suffix?)` | Generates `NodeID` from `(graphName, group, suffix)` and delegates to `addNode`. If `suffix` is null, counter increments and resulting value is used. If custom suffix is given, counter still increments to preserve monotonicity. | `group`: registered group name; `suffix`: optional custom suffix | `N` — the created node | `IllegalArgumentException` if group/suffix invalid or group not registered; `EntityAlreadyExistException` if generated ID exists |
...
| `getGroupNode(group, suffix)` | Constructs `NodeID` from `(graphName, group, suffix)` and delegates to `getNode` | `group`: group name; `suffix`: suffix string | `N?` — the node or null if not found | — |
...
```kotlin
class MyGraph(name: String) : AbcSimpleGraph<MyNode, MyEdge>(name), TraitNodeGroup<MyNode, MyEdge> {
...
- `graphName` is sanitized by stripping `@` characters to prevent ambiguous ID splitting
