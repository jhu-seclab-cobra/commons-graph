...
The storage layer is the **backend-agnostic directed property graph engine**. It manages nodes and edges identified by caller-provided `String` IDs, per-node and per-edge properties, adjacency indices (incoming/outgoing edge sets per node), edge endpoint metadata (source, destination, type), and graph-level metadata. It does not know about domain types (`Label`) — those belong to the graph and entity layers.

`IStorage` operates on caller-provided `String` IDs directly. Callers pass node IDs and edge IDs to `addNode()` and `addEdge()`. Storage implementations may internally map String IDs to integer indices for compactness and cache-friendly access — this is a pure implementation detail not exposed through the interface.

Caller-controlled IDs are a deliberate design choice: layered storage needs to create entries with specific IDs (during freeze merges and shadow entry creation). With auto-generated IDs, this would require internal methods (`addNodeWithId`, `setCounterStart`) that break encapsulation. With caller-controlled IDs, all layer operations work through the standard `IStorage` interface.

It provides two tiers of storage capability:

1. **Flat storage** (`NativeStorageImpl`, external `MapDBStorageImpl` / `JgraphtStorageImpl` / `Neo4jStorageImpl`) — single-layer, full CRUD, manages optional internal String→Int mapping
...
- `addEdge` stores edge structure (src, dst, type) internally, queryable via `getEdgeSrc`/`getEdgeDst`/`getEdgeType`, all returning `String`.
...
- `transferTo` performs ID preservation: String IDs are copied as-is to the target via `addNode`/`addEdge`.
- Caller-controlled IDs enable `LayeredStorageImpl` to perform freeze merges and shadow entry creation through the standard interface, eliminating the need for internal methods that break encapsulation.

---

### NativeStorageImpl

**Responsibility:** Pure in-memory `IStorage` implementation using `HashMap` (single-threaded). Manages optional internal String→Int mapping for compactness.
...
`EdgeEndpoints` is a `private data class` storing `(src: Int, dst: Int, type: String)` — the edge endpoint index using internal IDs.

- All operations are O(1) average.
- No synchronization — not thread-safe.
- String ID lookups are O(1) HashMap hits; internal integer ID operations use cache-friendly integer-keyed structures.
- When methods are called with String IDs, the implementation translates to internal integer ID operations transparently.
- When returning edge endpoints via `getEdgeSrc`/`getEdgeDst`, the implementation reverses the lookup from internal integer ID to String ID.
...
- No internal methods are exposed — all interaction is through the `IStorage` interface. Internal String→Int mapping is a pure implementation detail.
...
**Responsibility:** Multi-layer freeze-and-stack `IStorage` for phased analysis pipelines. Composes at most one frozen layer (read-only) + one mutable active layer. Writes target the active layer only. Deletion is restricted to the active layer — frozen layers are immutable. Depends only on the `IStorage` interface — does not access internal methods of concrete implementations.
...
**Write routing for cross-layer properties:** When `setNodeProperties` targets a node that exists only in the frozen layer, a shadow entry is created in `activeLayer` via `addNode(nodeId)` to hold the overlay properties. Same for edges via `addEdge(src, dst, edgeId, type)` (which copies edge structure from frozen layer using `getEdgeSrc`/`getEdgeDst`/`getEdgeType`). All shadow creation uses standard `IStorage` interface methods.
...
`mergeLayerInto` copies all nodes and edges from a source layer into the merged target using standard `addNode`/`addEdge` calls with preserved String IDs. If a node/edge already exists in the target (from a previous layer), properties are updated via `setNodeProperties`/`setEdgeProperties`. Edge reconstruction uses `source.getEdgeSrc`/`getEdgeDst`/`getEdgeType` to read endpoints from the source layer. No ID remapping or counter manipulation is needed — String IDs are preserved as-is.

**Design rationale:** Restricting deletion to the active layer eliminates deletion tracking sets and sentinel values. Merge-on-freeze ensures query resolution is always a simple 2-way lookup (active then single frozen), keeping query depth at O(1). Caller-controlled String IDs mean freeze and shadow operations use only the `IStorage` interface, requiring no internal methods or counter coordination between layers.
...
- Internal String→Int mapping is a pure implementation detail — no internal methods are exposed
- No `addNodeWithId`, `addEdgeWithId`, or `setCounterStart` methods exist — all operations use the standard `IStorage` interface
...
- String IDs are preserved across layer merges — no remapping needed
