...
internal class EntityPropertyMap<K : IEntity.ID>(
    ...
    inner class PropertyMap(
                ...
                override fun iterator() =
                    object : MutableIterator<MutableMap.MutableEntry<String, IValue>> {
                        ...
                        override fun remove() {
                            ...
                            // because the keys are constructed so that it will influence each others
        ...
        override val keys
            get() =
                object : MutableSet<String> {
                     ...
                     */
                    override fun contains(element: String): Boolean = element.let { eleKey -> this@PropertyMap[eleKey] != null }
                     ...
                     * @throws UnsupportedOperationException Always
                     */
                    override fun add(element: String): Boolean = throw UnsupportedOperationException("Cannot add to only key")
                    ...
                    override fun retainAll(elements: Collection<String>): Boolean {
                        val allPropKeys = identities[eid]!!.map { it.core.toString() }.toSet()
                     ...
                     * @return True if this property map was modified
                     */
                    override fun remove(element: String): Boolean = element.let { this@PropertyMap.remove(it) != null }
        ...
        override val values
            get() =
                object : MutableCollection<IValue> {
                    /**
                     ...
                     * @throws UnsupportedOperationException Always
                     */
                    override fun add(element: IValue): Boolean = throw UnsupportedOperationException("Cannot add to only values")

                    /**
                     * @return An iterator over the values in this property map
                     */
                    override fun iterator() =
                        object : MutableIterator<IValue> {
         ...
         * @throws NoSuchElementException If the entity does not exist
         */
        override fun isEmpty(): Boolean = identities[eid]?.isEmpty() ?: throw NoSuchElementException("Entity $eid does not exist")
    ...
    private inner class Entry(
        override val key: K,
    ) : MutableMap.MutableEntry<K, Map<String, IValue>> {
        /**
    ...
    override val entries
        get() =
            object : MutableSet<MutableMap.MutableEntry<K, Map<String, IValue>>> {
                /**
                 ...
                 */
                override fun containsAll(elements: Collection<MutableMap.MutableEntry<K, Map<String, IValue>>>) =
                 ...
                 */
                override fun iterator() =
                    object : MutableIterator<MutableMap.MutableEntry<K, Map<String, IValue>>> {
                        private var currentItem: K? = null
                        val entityIdIterator = identities.iterator()
     ...
     */
    override val values =
        object : MutableCollection<Map<String, IValue>> {
            /**
            ...
            override fun containsAll(elements: Collection<Map<String, IValue>>): Boolean {
                val allValues = identities.map { PropertyMap(it.key) }.toSet()
            ...
            override fun contains(element: Map<String, IValue>): Boolean {
                val allValues = identities.asSequence().map { PropertyMap(it.key) }
             ...
             * @throws UnsupportedOperationException Always
             */
            override fun add(element: Map<String, IValue>): Boolean = throw UnsupportedOperationException("Cannot add to only values")

            /**
             * @return An iterator over the property maps in this entity-property map
             */
            override fun iterator() =
                object : MutableIterator<Map<String, IValue>> {
                    private val entriesIterator = entries.iterator()
     ...
     */
    override fun get(key: K): Map<String, IValue>? = if (key !in identities) null else PropertyMap(key)
     ...
     */
    override fun containsValue(value: Map<String, IValue>) =
        identities.any { (eid, pKeys) ->
            if (value.keys.toSet() != pKeys.core.map { it.core.toString() }.toSet()) return@any false
            pKeys.core.all { propertiesMap["$eid:${it.core}"] == value[it.core.toString()] }
        }
