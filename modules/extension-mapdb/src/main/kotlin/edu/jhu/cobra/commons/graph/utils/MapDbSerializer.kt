...
class MapDbValSerializer<T : IValue>(
    ...
    override fun serialize(out: DataOutput2, value: T) {
        if (valueType == null) valueType = value::class
...
class MapDbEntityIDSerializer<T : IEntity.ID> : Serializer<T>, Serializable {
    private val core: IValSerializer<ByteArray> = DftByteArraySerializerImpl
     ...
     */
    override fun serialize(out: DataOutput2, value: T) {
        out.write(core.serialize(value = value.serialize))
    }
     ...
     */
    override fun deserialize(input: DataInput2, available: Int): T {
        return core.deserialize(input.asByteArray(available)).toEntityID()
    }
}
